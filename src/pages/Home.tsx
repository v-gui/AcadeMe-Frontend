...
// Interface para os dados que vêm do MongoDB
interface Aluno {
  _id: string;
  name: string;
  course: string;
...
const Home: React.FC = () => {
  ...
  // Estado para a Vitrine de Alunos
  const [alunos, setAlunos] = useState<Aluno[]>([]);

  // Busca os alunos do backend assim que a Home carrega
  useEffect(() => {
  ...
  return (
    <div className="Home relative overflow-x-hidden">
        ...
        <div className="top-container flex flex-col items-center justify-center text-center w-full">
          ...
          <div className="top-content flex flex-col items-center justify-center">
            ...
            <div className="top-buttons-container flex flex-col md:flex-row items-center justify-center gap-4 md:gap-[24px]">
              <Button size='default' shape='pill' className='min-w-[171px]' onClick={handleGoToSignUp}>
                Cadastre-se
              </Button>
              <Button size='default' shape='pill' className='min-w-[171px]' onClick={handleGoToLogin}>
                Login
              </Button>
            </div>
          </div>
      ...
      {/** 4. VITRINE DE ALUNOS (A MÁGICA DO BACKEND) **/}
      <section id="vitrine-section" className="py-20 bg-gray-50 flex flex-col items-center">
        <h1 className="text-[#006ACB] font-bold text-[28px] md:text-[40px] mb-12">Nossos Talentos</h1>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 px-8 max-w-7xl w-full">
          {alunos.map((aluno) => (
            <div key={aluno._id} className="bg-white p-6 rounded-2xl shadow-lg border-b-4 border-blue-500 hover:scale-105 transition">
              <h3 className="text-xl font-bold text-gray-800">{aluno.name}</h3>
              <p className="text-blue-600 font-semibold text-sm mb-3">{aluno.course}</p>
              <p className="text-gray-600 text-sm italic">"{aluno.bio}"</p>
            </div>
          ))}
          {alunos.length === 0 && <p className="col-span-3 text-center text-gray-400 italic">Aguardando novos talentos se cadastrarem...</p>}
        </div>
      </section>
